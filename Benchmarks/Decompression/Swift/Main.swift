...
#warning(
    """
    clock() function not imported for this platform, internal benchmarks not built (please open an issue at https://github.com/tayloraswift/swift-png/issues)
    """
)
#endif
...
extension Benchmark.Decode.Blob: PNG.BytestreamSource {
    static func load(path: String) -> Self? {
        System.File.Source.open(path: path) {
            ...
            let buffer: [UInt8]  = file.read(count: count) else {
                return nil
            }
...
extension Benchmark.Decode {
    static func rgba8(path: String, trials: Int) -> [(time: Int, hash: Int)] {
        guard var blob: Blob = .load(path: path) else {
...
func main() throws {
    ...
    let trials: Int  = Int.init(CommandLine.arguments[2]) else {
    ...
    let times: [Int] = __Entrypoint.Benchmark.Decode.rgba8(path: path, trials: trials).map(
        \.time
    )
    #else
    let times: [Int] =              Benchmark.Decode.rgba8(path: path, trials: trials).map(
        \.time
    )
    #endif

...
extension Benchmark.Encode {
    static func rgba8(level: Int, path: String, trials: Int) -> (
        [(time: Int, hash: Int)],
        Int
    ) {
...
func main() throws {
    ...
    let trials: Int  = Int.init(CommandLine.arguments[3]) else {
        fatalError(
            """
            usage: \(
                CommandLine.arguments.first ?? ""
            ) <compression-level:0 ... 9> <image> <trials>
            """
        )
    }

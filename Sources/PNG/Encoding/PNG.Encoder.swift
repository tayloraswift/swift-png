...
extension PNG.Encoder {
    ...
    static func filter(_ line: [UInt8], last: [UInt8], delay: Int) -> [UInt8] {
        ...
        let candidates: ([UInt8], [UInt8], [UInt8], [UInt8], [UInt8]) = (
            ...
            [4] + zip(current.prefix   (delay),                  last.prefix   (delay) ).map {
                ...
                + zip(
                current.dropFirst(delay),
                zip(current, zip(last.dropFirst(delay), last))
            ).map {
    ...
    private static func score<C>(_ filtered: C) -> Int
        where C: Sequence, C.Element == UInt8 {

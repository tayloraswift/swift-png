...
extension PNG {
    static let adam7: [(base: (x: Int, y: Int), exponent: (x: Int, y: Int))] = [
...
extension PNG.Decoder {
    ...
    mutating func push(
        _ data: [UInt8], size: (x: Int, y: Int), pixel: PNG.Format.Pixel,
        delegate: (UnsafeBufferPointer<UInt8>, (x: Int, y: Int), (x: Int, y: Int)) throws -> ()
    ) throws -> Void? {
        guard let _: Void = self.continue else {
            ...
            for z: Int in pass ..< 7 {
                ...
                for y: Int in start ..< subimage.y {
                    ...
                    print(
                        """
                        < scanline(\(scanline[0]))[\(
                            scanline.dropFirst().prefix(8).map(String.init(_:)).joined(
                                separator: ", "
                            )
                        ) ... ]
                        """

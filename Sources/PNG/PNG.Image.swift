extension PNG {
    ...
    public enum Data {
        ...
        public typealias Rectangular = PNG.Image
    }
}
extension PNG {
    /// A rectangular image.
    @frozen public struct Image {
        /// The size of this image, measured in pixels.
        public let size: (x: Int, y: Int)
        /// The layout of this image.
        public let layout: PNG.Layout
        /// The metadata in this image.
        public var metadata: PNG.Metadata
...
extension PNG.Image {
    init?(
        ...
        guard
        let layout: PNG.Layout = PNG.Layout.init(
            standard: standard,
            pixel: header.pixel,
            palette: palette,
    ...
    public func bindStorage(to layout: PNG.Layout) -> Self {
        ...
        case    (.indexed1(palette: let old, fill: _), .indexed1(palette: let new, fill: _)),
            ...
            guard old.count == new.count else {
                fatalError(
                    """
                    new palette count (\(new.count)) must match old palette count (\(old.count))
                    """
                )
            }
        ...
        default:
            fatalError(
                """
                new pixel format (\(layout.format.pixel)) must match old pixel format (\(
                    self.layout.format.pixel
                ))
                """
            )
        }
        ...
        return .init(
            size: self.size, layout: layout, metadata: self.metadata,
    ...
    private mutating func overdraw<T>(
        at base: (x: Int, y: Int),
        brush: (x: Int, y: Int),
        element: T.Type
    ) {
    ...
    mutating func assign<C>(scanline: C, at base: (x: Int, y: Int), stride: Int)
    ...
    public static func decompress<Source>(stream: inout Source) throws -> Self
        ...
        var context: PNG.Context = try {
            ...
            while true {
                ...
                default:
                    try metadata.push(
                        ancillary: chunk, pixel: header.pixel,
                        palette: palette,
                        background: &background,
                        transparency: &transparency
                    )
...
extension PNG.Image {
    ...
    public func compress<Destination>(
        ...
        var encoder: PNG.Encoder = .init(
            standard: cgbi == nil ? .common : .ios,
            interlaced: self.layout.interlaced, level: level, hint: hint
        )
        while let data: [UInt8] = encoder.pull(
                size: self.size,
                pixel: self.layout.format.pixel,
...
extension PNG.Image {
    ...
    @usableFromInline @_specialize(where T == UInt8)
    @_specialize(where T == UInt16)
    @_specialize(where T == UInt32)
    @_specialize(where T == UInt64)
    @_specialize(where T == UInt)
    static func unpack<T>(
        _ interleaved: [UInt8], of format: PNG.Format, as _: T.Type,
    ...
    @_specialize(where T == UInt)
    static func pack<T>(
        _ pixels: [T], as format: PNG.Format,
...
extension PNG.Image {
    ...
    @inlinable public func unpack<Color>(
        as _: Color.Type,
    ...
    ///     first pixel in this array corresponds to the top-left corner of
    ///     the image. Its length is equal to `size.x` multiplied by `size.y`.
    @inlinable public func unpack<T>(
        as _: T.Type,
        deindexer: ([(r: UInt8, g: UInt8, b: UInt8, a: UInt8)]) -> (Int) -> UInt8
    )
    -> [T]

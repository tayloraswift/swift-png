extension PNG {
    ...
    public struct Chunk: Hashable, Equatable, CustomStringConvertible {
        /// The chunk type code.
        ...
        private init(unchecked name: UInt32) {
            self.name = name
        }
        ...
        public init(name: UInt32) {
            guard let chunk: Self = Self.init(validating: name) else {
        ...
        ///     The chunk type code. Bit 13 must be set. If the type code is not
        ///     a public PNG chunk type code, then bit 29 must be clear.
        public init?(validating name: UInt32) {

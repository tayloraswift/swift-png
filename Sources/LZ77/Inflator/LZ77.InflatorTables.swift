extension LZ77 {
    @frozen @usableFromInline struct InflatorTables {
        private typealias Composite = (extra: UInt16, base: UInt16)

        private let storage: ManagedBuffer<Void, UInt8>,
...
extension LZ77.InflatorTables {
    init(literals: LZ77.HuffmanTree<UInt16>, distances: LZ77.HuffmanTree<UInt8>) {
        ...
        self.storage.withUnsafeMutablePointerToElements {
            ...
            (base +  start).withMemoryRebound(
                to: LZ77.RunLiteral.self,
                capacity: literals.size.z
            ) {
                literals.table(initializing: $0)
            }
            (base + offset).withMemoryRebound(
                to: LZ77.Distance.self,
    ...
    static let fixed: Self = .init(literals: .runliteral, distances: .distance)
}

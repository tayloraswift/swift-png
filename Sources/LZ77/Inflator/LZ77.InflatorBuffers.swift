extension LZ77 {
    @frozen @usableFromInline struct InflatorBuffers<Format> where Format: LZ77.FormatType {
        //  Reusable buffers
    ...
    mutating func advance(state: LZ77.InflatorState) throws -> LZ77.InflatorState? {
        ...
        case .initial:
            if  case .ios = self.format {
                ...
                let header: LZ77.StreamHeader = try .read(
                    &self.stream.input,
                    from: &self.stream.b
                ) {
...
extension LZ77.InflatorBuffers<Gzip.Format> {
    mutating func advance(state: Gzip.InflatorState) throws -> Gzip.InflatorState? {
        ...
        switch state {
        case .initial:
            if  let header: Gzip.StreamHeader = try .read(
                    &self.stream.input,
                    from: &self.stream.b
                ) {
                self.stream.output.window = 1 << 15

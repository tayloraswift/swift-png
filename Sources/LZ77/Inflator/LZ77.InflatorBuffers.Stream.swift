extension LZ77.InflatorBuffers {
    @frozen @usableFromInline struct Stream {
...
extension LZ77.InflatorBuffers.Stream {
    mutating func readBlockMetadata(
        into metadata: inout LZ77.BlockMetadata
    ) throws -> LZ77.BlockShape? {
        ...
        case 2:
            ...
            guard
            let tree: LZ77.HuffmanTree<UInt8> = .validate(
                symbols: 0 ... 18,
                lengths: lengths
            ) else {
    ...
    mutating func readBlockTables(
        ...
        while lengths.count < count.total {
            ...
            case 16:
                guard
                let last: Int = lengths.last else {
        ...
        guard
        let literalTree: LZ77.HuffmanTree<UInt16> = .validate(
            symbols: 0 ... 287,
            lengths: lengths.prefix(count.literals)
        ),
        let distanceTree: LZ77.HuffmanTree<UInt8> = .validate(
            symbols: 0 ... 31,
    ...
    @inline(__always) mutating func readByte() -> UInt8? {
    ...
    mutating func _dumpPerfStats() {
        #if DUMP_LZ77_BLOCKS
        let efficiency: Double = self.statistics.literals.enumerated().reduce(0.0){
            $0 + .init($1.0 * $1.1)
        } /

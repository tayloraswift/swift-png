extension LZ77 {
    @frozen public struct InflatorIn {
...
extension LZ77.InflatorIn {
    ...
    @inline(__always) private static func atoms(bytes: Int) -> Int {
    ...
    mutating func rebase(_ data: ArraySlice<UInt8>, pointer b: inout Int) {
        ...
        if !isKnownUniquelyReferenced(&self.storage) {
            print(
                """
                warning: managed buffer in type '\(
                    String.init(reflecting: Self.self)
                )' has multiple references; buffer is being copied to preserve value semantics
                """
            )
        }
        #endif

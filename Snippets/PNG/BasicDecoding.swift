...
guard
let image: PNG.Image = try .decompress(path: "\(path).png") else {
...
guard
let _: Void = (
    System.File.Destination.open(path: "\(path).png.rgba") {
        guard
        let _: Void = $0.write(rgba.flatMap{ [$0.r, $0.g, $0.b, $0.a] }) else {
...
let vaReencoded: PNG.Image = .init(
    packing: va,
    size: image.size,
